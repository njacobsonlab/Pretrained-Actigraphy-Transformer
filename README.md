...
Abstract: Wearable accelerometry (actigraphy) has provided valuable data for clinical insights since the 1970s and is increasingly important as wearable devices continue to become more widely utilized. The effectiveness of actigraphy in research and clinical contexts depends heavily on the modeling architectures used to analyze the data. To address this, we developed the Pretrained Actigraphy Transformer (PAT)—the first pretrained and fully attention-based model designed specifically to handle actigraphy. PAT was pretrained on actigraphy data from 29k participants in NHANES, enabling it to deliver state-of-the-art performance when fine-tuned across various actigraphy prediction tasks in the mental health domain, even in data-limited scenarios. For example, when predicting benzodiazepine usage using actigraphy data from only labeled 500 participants, PAT achieved a 9.4 percentage-point absolute improvement over the best baseline model. With fewer than 2 million parameters and built-in model explainability, PAT is robust yet easy to deploy in health research settings. <br>
